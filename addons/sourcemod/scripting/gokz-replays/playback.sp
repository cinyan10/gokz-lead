...
public bool GetReplayTickData(int bot, int tick, any output[RP_V2_TICK_DATA_BLOCKSIZE])
{
	if (bot < 0 || bot >= RP_MAX_BOTS) return false;
	if (tick < 0 || tick >= playbackTickData[bot].Length) return false;

	if (botReplayVersion[bot] == 2)
	{
		ReplayTickData tickData;
		if (!playbackTickData[bot].GetArray(tick, tickData)) return false;
		TickDataToArray(tickData, output);
		return true;
	}
	else if (botReplayVersion[bot] == 1)
	{
		output[0]  = 0; output[1]  = 0; // flags
		output[2]  = 0.0; output[3] = 0.0; output[4] = 0.0; // vel
		output[5]  = 0.0; output[6] = 0.0;
		output[7]  = playbackTickData[bot].Get(tick, 0); // origin.x
		output[8]  = playbackTickData[bot].Get(tick, 1);
		output[9]  = playbackTickData[bot].Get(tick, 2);
		output[10] = playbackTickData[bot].Get(tick, 3); // angles.x
		output[11] = playbackTickData[bot].Get(tick, 4);
		output[12] = 0.0;
		output[13] = 0.0; output[14] = 0.0; output[15] = 0.0;
		output[16] = 0; output[17] = 0;
		output[18] = 0.0; output[19] = 0;
		return true;
	}
	return false;
}
...
int LoadReplayBot(int client, char[] path, bool isAuto = false)
{
	// Safeguard Check
...
public Action AutoReplay_RestartTimer(Handle timer, any botIndex)
{
    char map[PLATFORM_MAX_PATH];
    GetCurrentMap(map, sizeof(map));

    char filePath[PLATFORM_MAX_PATH];
    Format(filePath, sizeof(filePath), "addons/sourcemod/data/gokz-replays/_runs/%s/0_KZT_NRM_PRO.replay", map);

    // 内联 FindFirstHumanClient()
    int client = 0;
    ...
    if (client > 0 && FileExists(filePath))
    {
        GOKZ_RP_LoadJumpReplay(client, filePath, true);
        PrintToServer("[AutoReplay] Restarted replay for map: %s", map);
    }
    else
    {
        PrintToServer("[AutoReplay] Failed to restart replay: no human client or replay file not found.");
    }
